...
In the plots that follow, the black lines are the observed number of arrests, while the solid gray line is what the model predicted. The gray shading shows a point-wise 95% posterior credible interval for the prediction - it can be thought of as the range of predictions that the model believes are consistent with the data. It does not include the residual error term, which is what would cause an observation to be randomly above or below the prediction.
...
Finally, as the above were based on a fitted model that cannot accurately describe the data in all circumstances, we include an ad-hoc approach that simply looks at those jurisdictions with the largest and smallest increases from an average number of arrests. These plots show those with years that are the smallest percentage-wise from the average.

`r rmdImageInline(file.path(imgPath, "analysis_numArrestsDownJumps.pdf"))`

`r rmdPageBreak()`

Conversely, these are those with years having the largest percent increases.
