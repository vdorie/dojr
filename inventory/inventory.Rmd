...
loadData <- function(tableName = mainTable) {
  ...
  if (!dataLoaded[[tableName]]) {
    ...
    if (tables$on_load[tableIndex] != "") eval(parse(text = tables$on_load[tableIndex]), .GlobalEnv)
  }
...
`r if (targetType == "latex") cat("\n\\newpage\n\n")`

## Main Table Variables

```{r variables, echo=FALSE}
variables <- read.csv(file.path(datasetDir, "macr_variables.csv"), stringsAsFactors = FALSE, header = TRUE)
```
