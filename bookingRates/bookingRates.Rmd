...
Although a 100% booking rate is implausible some smaller jurisdictions may have very high booking rates due to small sample sizes. Arbitrarily, we proceed by excluding those that approach 100% three or more times. Another simple cut can be made by excluding jursidictions that are mostly missing, such as those for which have only one recorded value. `r rmdColFmt("red", "I think it makes sense to also look at the min/max sample sizes.")`
...
While some of these may certainly be valid, we again arbitrarily exclude them from further analysis. `r rmdColFmt("red", "Again, I don't feel great about excluding all of these, but with 21 jurisdictions it might be OK to look at them individually.")`
...
Because the rates for these felonies should be high, we highlight any jursidiction that, for two or more years, had a booking rate lower than 80%.
...
intersection <- read.csv("intersection.csv", stringsAsFactors = FALSE)
...
## TODO: fix double booked, error bars on proportion estimates
...
At this point, the `r nrow(br.adult.yearly)` jurisdictions that remain seem to be adequate in terms of variabilty. Their booking rates for juvenile misdemeanors are:
