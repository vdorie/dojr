...
We can look at the distribution of ages in the data by subtracting the court event date from the birth date.

```{r 'clean_birth_date_3', echo = FALSE}
imgFile <- file.path("..", imgPath, "clean_age.png")
if (performClean) {
  age <- with(obts.clean, court_event_year - birth_year + ifelse(court_event_month > birth_month | court_event_month == birth_month & court_event_day >= birth_day, 0L, 1L))
  pdf(imgFile, 6, 3.5)
  par(mfrow = c(1L, 2L))
  discrete.histogram(age[!is.na(age) & age < 18L], main = "Juvenile Discrete Histogram", col = "gray", border = NA, xlab = "age")
  discrete.histogram(age[!is.na(age) & age < 18L], main = "Juvenile Disc Hist Closeup", col = "gray", border = NA, xlab = "age", ylim = c(0, 125))
  dev.off()
  rm(age)
}
rm(imgFile)
...
`r rmdImageInline(file.path(imgPath, "clean_age.png"))`
