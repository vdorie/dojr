...
test_that("fails with invalid mcmc inputs", {
  ...
  if (FALSE) { ## mcmc inputs not in use
...
test_that("returns trivial solutions", {
  ...
  res <- localSuppression(testData, risk.k = 2, n.chains = 1L)
  expect_true(all(is.na(res$x[,c("gender", "age_group", "race")])))
  
  ## already above threshold
  res <- localSuppression(testData, risk.k = 1, n.chains = 1L)
  expect_equal(res$x[,c("gender", "age_group", "race")], testData)
  
  ## not enough rows
  res <- localSuppression(testData, risk.k = 6, n.chains = 1L)
  expect_true(all(is.na(res$x[,c("gender", "age_group", "race")])))
})
